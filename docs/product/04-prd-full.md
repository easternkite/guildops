...
| 항목 | baseline(현재값) | 목표 값 (V1) |
|---|---:|---:|
| 주간 활성 길드(WAG) | 미계측 | 50+ |
| 출석 이벤트당 체크인 완결률 | 미계측 | 85%+ |
| Free→Paid 전환율 | 미계측 | 5%+ |
| 운영자 주간 업무시간 절감 | 미계측 | 40%+ |
...
| KPI | 정의 | baseline(현재값) | 수집 주기 |
|---|---|---:|---|
| WAU (길드) | 7일 내 1회 이상 운영 액션 수행 길드 수 | 0 (계측 시작 전) | 주간 |
| 출석 생성 수 | 주간 생성된 attendance 건수 | 0 (계측 시작 전) | 주간 |
| 체크인 완료율 | 체크인 완료 인원 / 대상 인원 | 0% (계측 시작 전) | 주간 |
| 운영 리텐션 | 4주 연속 활성 길드 비율 | 0% (계측 시작 전) | 월간 |
